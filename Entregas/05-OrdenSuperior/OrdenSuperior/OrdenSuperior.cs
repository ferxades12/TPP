...
public class Program
{
    ...
    public static void EjercicioTransacciones()
    {
        ...
        int total = Reduce(
            Filter(
                historicoVentas,
                v => Equals(v.Region, "NorteAmérica") && Equals(v.Estado, Estado.Cancelada)
            ),
            (v, acc) => acc + 1,
            0
        );
        ...
        var resultado = Zip(
            ...
            (r, m) =>
            {
                var facturacionNeta = Reduce(
                    Filter(
                        historicoVentas,
                        v => Equals(v.Region, r) && Equals(v.Estado, Estado.Confirmada)
                    ),
        ...
        Console.WriteLine(
            $"Región con mayor facturación neta: {regionMayorFacturacion.Region}, Importe: {regionMayorFacturacion.FacturacionNeta}"
        );
    }
