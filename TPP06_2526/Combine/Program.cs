...
static class Program
{
    ...
    public static Func<IEnumerable<T1>, IEnumerable<T2>> MapC<T1, T2>(Func<T1, T2> f)
    {
        return xs => Map(xs, f);
    }

    public static Func<IEnumerable<T>, Predicate<T>> FilterC<T>(Predicate<T> f)
    {
        return xs => Filter<T>(xs, f);
    }

    public static Func<IEnumerable<T1>, Func<:>> ReduceC<T1, T2>(T2 seed, Func<T2, T1, T2> f)
    {
        return xs => Filter<T>(xs, f);
    }
